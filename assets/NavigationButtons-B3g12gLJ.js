import{d as e,j as o,r as g,S as h,u as d,a as x,B as a,L as u}from"./index-Bb_VmvaQ.js";const y=e.div`
  width: 100%;
  position: relative;
`,S=e.div`
  position: absolute;
  top: 5%;
  right: 5%;
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: 2;
`,f=e.div`
  width: 2px;
  height: 26px;
  margin-inline: 3px;
  background-color: ${t=>t.theme.colors.primary};
`,D=e.div`
  width: 80%;
  margin: 0 auto;
  margin-top: 50px;
  margin-bottom: 10px;

  @media (min-width: 1024px) {
    width: 55%;
    height: 100%;
    display: flex;
    flex-direction: column;
    justify-content: center;
    margin-bottom: 0;
    margin-top: 0;
  }
`,w=e.div`
  width: 100%;
  display: flex;
  align-items: flex-start;
  margin-top: 50px;
  gap: 10px;
`,J=e.div`
  display: grid;
  grid-template-columns: repeat(
    ${t=>t.$columns?t.$columns:2},
    1fr
  );
  gap: 5px;
  margin: 0 auto;
  width: 100%;

  @media (min-width: 1024px) {
    grid-template-columns: repeat(
      ${t=>t.$columns?parseInt(t.$columns,10)+1:3},
      1fr
    );
  }
`,H=e.div`
  @media (max-width: 1023px) {
    &:nth-child(odd):last-child {
      grid-column: 1 / -1;
    }
  }
`,Y=e.main`
  display: flex;
  flex-direction: column;
  gap: 10px;
  width: 100%;

  @media (min-width: 1024px) {
    flex-direction: row;
    background-color: ${t=>t.$isLastStep?t.theme.colors.blue:""};
  }
`,F=e.div`
  width: 100%;

  @media (min-width: 1024px) {
    width: 50%;
    order: 2;
  }
`,z=e.div`
  width: 100%;

  @media (min-width: 1024px) {
    width: 50%;
    order: 1;
  }
`,G=e.div`
  height: 4px;
  width: ${t=>t.$progress};
  background-color: ${t=>t.theme.colors.primary};
  position: fixed;
  left: 0;
  top: 0;
  z-index: 20;

  @media (min-width: 1024px) {
    height: 8px;
  }
`,b=e.img`
  width: 100%;
  object-fit: cover;
  object-position: center;

  @media (min-width: 1024px) {
    display: none;
  }
`,j=e.img`
  position: absolute;
  top: 50%;
  left: 25%;
  transform: translate(-50%, -50%);

  @media (min-width: 1024px) {
    display: none;
  }
`,C=e.img`
  width: 100%;
  height: 100vh;
  /* object-position: center; */
  display: none;

  @media (min-width: 1024px) {
    display: block;
  }
`,R=e.img`
  display: none;
  width: 113px;
  height: 41px;
  margin-bottom: 30px;
  @media (min-width: 1024px) {
    display: block;
  }
`,E=e.img`
  display: none;
  @media (min-width: 1024px) {
    display: ${t=>t.$isFirstORLastPage?"none":"block"};
  }
`,I="/assets/banner_mobile-CBl6AMHh.png",O="/assets/banner-desktop-oJIU8P16.png",l="data:image/png;base64,",c=e.p`
  font-size: 18px;
  line-height: 21px;
  font-weight: 700;
  color: ${t=>t.theme.colors.primary};
`,B=e(c)`
  font-size: 10px;
  line-height: 12px;
`,Q=e.p`
  text-align: center;
  font-size: 11px;
  flex: 1;
`,v=e.p`
  font-size: 19px;
  font-weight: 900;
  line-height: 24px;
  color: ${t=>t.theme.colors.primary};
  text-align: left;
`,q=e(v)`
  line-height: 39px;
  font-size: 30px;
`,K=e.p`
  font-style: oblique;
  font-size: 20px;
  line-height: 26px;
  color: ${t=>t.theme.colors.primary};
  text-align: left;
  margin-top: 40px;
  font-weight: 500;
`,P=e.p`
  font-weight: 400;
  font-size: ${t=>t.theme.text.medium};
  line-height: 24px;
  color: ${t=>t.theme.colors.primary};
  text-align: left;

  @media (min-width: 1024px) {
    font-size: ${t=>t.$isLastStep?"30px":"19px"};
    line-height: ${t=>t.$isLastStep?"39px":"19px"};
  }
`,W=e(P)`
  margin-top: ${t=>t.$spacing?"50px":"0px"};
  margin-bottom: 50px;
`,Z=e.span`
  font-weight: 900;
`,$=({pageNumber:t})=>o.jsxs(y,{style:{paddingLeft:"20px"},children:[o.jsx(b,{src:I,alt:"Banner mobile"}),o.jsx(C,{src:O,alt:"Banner desktop"}),o.jsx(j,{src:l,alt:"Logo Customer Scoops"}),o.jsxs(S,{children:[o.jsx(c,{children:t}),o.jsx(f,{}),o.jsx(B,{children:"06"})]})]}),_=()=>o.jsx(R,{src:l,alt:"logo Customer Scoops"}),U=e.button`
  width: 100px;
  height: 35px;
  color: white;
  font-size: ${t=>t.theme.text.small};
  background-color: ${t=>t.theme.colors.primary};
  border: none;
  border-radius: ${t=>t.theme.borderRadius};
  line-height: 13px;
  cursor: pointer;

  @media (min-width: 1024px) {
    width: 140px;
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 20px;
  }
`,X=e.div`
  width: 35px;
  height: 35px;
  border: 2px solid ${t=>t.theme.colors.primary};
  border-radius: 100%;
  display: flex;
  justify-content: center;
  align-items: center;
  cursor: pointer;
`,tt=e.div`
  width: 100%;
  height: 39px;
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: ${t=>t.selected?"white":t.theme.colors.primary};
  font-size: ${t=>t.theme.text.small};
  background-color: ${t=>t.selected?t.theme.colors.primary:"transparent"};
  border: 2px solid ${t=>t.theme.colors.primary};
  border-radius: ${t=>t.theme.borderRadius};
  line-height: 13px;
  cursor: pointer;
`,et=e.div`
  width: 50px;
  height: 85%;
  display: flex;
  justify-content: center;
  align-items: center;
  background-color: ${t=>t.theme.colors.blue};
  color: white;
  border-radius: ${t=>t.theme.borderRadius};
  font-weight: 900;
  margin-left: 5px;
  text-transform: uppercase;
  font-size: ${t=>t.theme.text.small};

  @media (min-width: 1024px) {
    width: 30px;
    height: 30px;
  }
`,V="data:image/png;base64,",k=()=>{const t=g.useContext(h);if(!t)throw new Error("useSurvey debe ser usado dentro de un SurveyProvider");return t},M=({text:t,nextPage:i,isFirstORLastPage:n})=>{const s=d(),A=x(),{state:p}=k(),m=()=>{const r=s.pathname.split("/")[1];if(p[r]||r==="step-6"){A(i),a.dismiss(r);return}a("Por favor completa la información",{position:"bottom-left",autoClose:3e3,hideProgressBar:!1,closeOnClick:!0,pauseOnHover:!0,draggable:!0,progress:void 0,theme:"colored",type:"info",toastId:r})};return o.jsxs(U,{onClick:m,children:[t,o.jsx(E,{src:V,alt:"arrow",$isFirstORLastPage:n})]})},T="data:image/png;base64,",L=({link:t})=>{const i=d(),n=()=>{const s=i.pathname.split("/")[1];a.dismiss(s)};return o.jsx(u,{to:t,children:o.jsx(X,{onClick:n,children:o.jsx("img",{src:T,alt:"arrow"})})})},ot=({text:t,isFirstORLastPage:i,nextPage:n,previousPage:s})=>o.jsxs(w,{children:[!i&&o.jsx(L,{link:s||""}),o.jsx(M,{text:t,nextPage:n,isFirstORLastPage:i})]});export{$ as B,Y as C,F,H as I,_ as L,ot as N,G as P,z as S,D as a,P as b,Z as c,W as d,v as e,tt as f,et as g,Q as h,J as i,q as j,K as k,k as u};

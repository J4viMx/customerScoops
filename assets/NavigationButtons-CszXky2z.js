import{d as e,j as o,r as g,S as h,u as d,a as x,B as a,L as u}from"./index-nulQdsSc.js";const y=e.div`
  width: 100%;
  position: relative;
`,S=e.div`
  position: absolute;
  top: 5%;
  right: 5%;
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: 2;
`,f=e.div`
  width: 2px;
  height: 26px;
  margin-inline: 3px;
  background-color: ${t=>t.theme.colors.primary};
`,Y=e.div`
  width: 80%;
  margin: 0 auto;
  margin-top: 50px;
  margin-bottom: 10px;

  @media (min-width: 1024px) {
    width: 55%;
    height: 100%;
    display: flex;
    flex-direction: column;
    justify-content: center;
    margin-bottom: 0;
    margin-top: 0;
  }
`,w=e.div`
  width: 100%;
  display: flex;
  align-items: flex-start;
  margin-top: 50px;
  gap: 10px;
`,H=e.div`
  display: grid;
  grid-template-columns: repeat(
    ${t=>t.$columns?t.$columns:2},
    1fr
  );
  gap: 5px;
  margin: 0 auto;
  width: 100%;

  @media (min-width: 1024px) {
    grid-template-columns: repeat(
      ${t=>t.$columns?parseInt(t.$columns,10)+1:3},
      1fr
    );
  }
`,F=e.div`
  @media (max-width: 1023px) {
    &:nth-child(odd):last-child {
      grid-column: 1 / -1;
    }
  }
`,z=e.main`
  display: flex;
  flex-direction: column;
  gap: 10px;
  width: 100%;

  @media (min-width: 1024px) {
    flex-direction: row;
    background-color: ${t=>t.$isLastStep?t.theme.colors.blue:""};
  }
`,G=e.div`
  width: 100%;

  @media (min-width: 1024px) {
    width: 50%;
    order: 2;
  }
`,Q=e.div`
  width: 100%;

  @media (min-width: 1024px) {
    width: 50%;
    order: 1;
  }
`,q=e.div`
  height: 4px;
  width: ${t=>t.$progress};
  background-color: ${t=>t.theme.colors.primary};
  position: fixed;
  left: 0;
  top: 0;
  z-index: 20;

  @media (min-width: 1024px) {
    height: 8px;
  }
`,b=e.img`
  width: 100%;
  object-fit: cover;
  object-position: center;

  @media (min-width: 1024px) {
    display: none;
  }
`,j=e.img`
  position: absolute;
  top: 50%;
  left: 25%;
  transform: translate(-50%, -50%);

  @media (min-width: 1024px) {
    display: none;
  }
`,C=e.img`
  width: 100%;
  height: 100vh;
  display: none;

  @media (min-width: 1024px) {
    display: block;
  }
`,R=e.img`
  display: none;
  width: 113px;
  height: 41px;
  margin-bottom: 30px;
  @media (min-width: 1024px) {
    display: block;
  }
`,E=e.img`
  display: none;
  @media (min-width: 1024px) {
    display: ${t=>t.$isFirstORLastPage?"none":"block"};
  }
`,I="/assets/banner_mobile-CBl6AMHh.png",O="/assets/banner-desktop-oJIU8P16.png",B="data:image/png;base64,",l=e.p`
  font-size: 18px;
  line-height: 21px;
  font-weight: 700;
  color: ${t=>t.theme.colors.primary};
`,U=e(l)`
  font-size: 10px;
  line-height: 12px;
`,K=e.p`
  text-align: center;
  font-size: 11px;
  flex: 1;
`,v=e.p`
  font-size: 19px;
  font-weight: 900;
  line-height: 24px;
  color: ${t=>t.theme.colors.primary};
  text-align: left;
`,W=e(v)`
  line-height: 39px;
  font-size: 30px;
`,Z=e.p`
  font-style: oblique;
  font-size: 20px;
  line-height: 26px;
  color: ${t=>t.theme.colors.primary};
  text-align: left;
  margin-top: 40px;
  font-weight: 500;
`,P=e.p`
  font-weight: 400;
  font-size: ${t=>t.theme.text.medium};
  line-height: 24px;
  color: ${t=>t.theme.colors.primary};
  text-align: left;

  @media (min-width: 1024px) {
    font-size: ${t=>t.$isLastStep?"30px":"19px"};
    line-height: ${t=>t.$isLastStep?"39px":"19px"};
  }
`,$=e(P)`
  margin-top: ${t=>t.$spacing?"50px":"0px"};
  margin-bottom: 50px;
`,_=e.span`
  font-weight: 900;
`,tt=({pageNumber:t})=>o.jsxs(y,{style:{paddingLeft:"20px"},children:[o.jsx(b,{src:I,alt:"Banner mobile"}),o.jsx(C,{src:O,alt:"Banner desktop"}),o.jsx(j,{src:B,alt:"Logo Customer Scoops"}),o.jsxs(S,{children:[o.jsx(l,{children:t}),o.jsx(f,{}),o.jsx(U,{children:"06"})]})]}),X="/assets/logo-desktop-KACyaDrl.png",et=()=>o.jsx(R,{src:X,alt:"logo Customer Scoops"}),V=e.button`
  width: 100px;
  height: 35px;
  color: white;
  font-size: ${t=>t.theme.text.small};
  background-color: ${t=>t.theme.colors.primary};
  border: none;
  border-radius: ${t=>t.theme.borderRadius};
  line-height: 13px;
  cursor: pointer;

  @media (min-width: 1024px) {
    width: 140px;
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 20px;
  }
`,k=e.div`
  width: 35px;
  height: 35px;
  border: 2px solid ${t=>t.theme.colors.primary};
  border-radius: 100%;
  display: flex;
  justify-content: center;
  align-items: center;
  cursor: pointer;
`,ot=e.div`
  width: 100%;
  height: 39px;
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: ${t=>t.selected?"white":t.theme.colors.primary};
  font-size: ${t=>t.theme.text.small};
  background-color: ${t=>t.selected?t.theme.colors.primary:"transparent"};
  border: 2px solid ${t=>t.theme.colors.primary};
  border-radius: ${t=>t.theme.borderRadius};
  line-height: 13px;
  cursor: pointer;
`,it=e.div`
  width: 50px;
  height: 85%;
  display: flex;
  justify-content: center;
  align-items: center;
  background-color: ${t=>t.theme.colors.blue};
  color: white;
  border-radius: ${t=>t.theme.borderRadius};
  font-weight: 900;
  margin-left: 5px;
  text-transform: uppercase;
  font-size: ${t=>t.theme.text.small};

  @media (min-width: 1024px) {
    width: 30px;
    height: 30px;
  }
`,T="data:image/png;base64,",M=()=>{const t=g.useContext(h);if(!t)throw new Error("useSurvey debe ser usado dentro de un SurveyProvider");return t},L=({text:t,nextPage:i,isFirstORLastPage:n})=>{const s=d(),c=x(),{state:A,dispatch:p}=M(),m=()=>{const r=s.pathname.split("/")[1];if(A[r]){c(i),a.dismiss(r);return}if(r==="step-6"){p({type:"RESET_SURVEY"}),window.location.href="https://www.customerscoops.com/";return}a("Por favor completa la información",{position:"bottom-left",autoClose:3e3,hideProgressBar:!1,closeOnClick:!0,pauseOnHover:!0,draggable:!0,progress:void 0,theme:"colored",type:"info",toastId:r})};return o.jsxs(V,{onClick:m,children:[t,o.jsx(E,{src:T,alt:"arrow",$isFirstORLastPage:n})]})},N="data:image/png;base64,",D=({link:t})=>{const i=d(),n=()=>{const s=i.pathname.split("/")[1];a.dismiss(s)};return o.jsx(u,{to:t,children:o.jsx(k,{onClick:n,children:o.jsx("img",{src:N,alt:"arrow"})})})},nt=({text:t,isFirstORLastPage:i,nextPage:n,previousPage:s})=>o.jsxs(w,{children:[!i&&o.jsx(D,{link:s||""}),o.jsx(L,{text:t,nextPage:n,isFirstORLastPage:i})]});export{tt as B,z as C,G as F,F as I,et as L,nt as N,q as P,Q as S,Y as a,P as b,_ as c,$ as d,v as e,ot as f,it as g,K as h,H as i,W as j,Z as k,M as u};
